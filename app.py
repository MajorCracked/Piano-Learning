...
if uploaded_file is not None:
    ...
    if st.button("Generate Transcription", use_container_width=True):
        if isolate_stems:
            with st.spinner("Isolating the piano... (Takes 2-3 mins)"):
                processing_path = isolate_piano_stem(temp_audio_path)
        else:
            processing_path = temp_audio_path
            
        with st.spinner("Calculating the notes... (Takes 2-3 mins)"):
        ...
        st.success("Multi-Stage AI Pipeline Complete! Rendering visualizer...")
